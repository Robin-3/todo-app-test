...
async fn model_todo_create() -> Result<(), Box<dyn std::error::Error>> {
    ...
    // -- CHECK
    assert!(todo_created.id_todo >= 1000, "Id todo should be >= 1000");
...
async fn model_todo_update_ok() -> Result<(), Box<dyn std::error::Error>> {
    ...
    // -- ACTION
    let todo_updated = TodoMac::update(&db, &utx, todo_fx.id_todo, update_data_fx.clone()).await?;

    // -- CHECK
    let todos = TodoMac::list(&db, &utx).await?;
    assert_eq!(3, todos.len());
    assert_eq!(todo_fx.id_todo, todo_updated.id_todo);
...
#[tokio::test]
async fn model_todo_update_staus_ok() -> Result<(), Box<dyn std::error::Error>> {
    // -- FIXTURE
    let db = init_db().await?;
    let utx = utx_from_token(&db, "123").await?;
    let data_fx = TodoPatch {
...
#[tokio::test]
async fn model_todo_update_wong_id() -> Result<(), Box<dyn std::error::Error>> {
    // -- FIXTURE
    let db = init_db().await?;
    let utx = utx_from_token(&db, "123").await?;
    let update_data_fx = TodoPatch {
    ...
    // -- ACTION
    let result = TodoMac::update(&db, &utx, 999, update_data_fx.clone()).await;
...
async fn model_todo_list() -> Result<(), Box<dyn std::error::Error>> {
    ...
    assert_eq!(123, todos[0].id_creator);
    assert_eq!("todo 101", todos[0].title);
    // todo 100
    assert_eq!(100, todos[1].id_todo);
    assert_eq!(123, todos[1].id_creator);
